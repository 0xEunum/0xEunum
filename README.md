<h1 align="center">Hi, I'm Mohd Muzammil 👋</h1>

<h3 align="center">Smart Contract Developer | DeFi & Security</h3>

---

## 📚 About Me

Final-year student building secure, gas-efficient DeFi primitives and oracle-enabled protocols with Solidity & Foundry on EVM-compatible chains.

- Build protocol-level smart contracts with security-first mindset
- Strong focus on testing, invariants, and gas efficiency
- Comfortable with EVM internals, standards, and common DeFi patterns

### Currently working on

- Small protocol-focused projects: upgradability, account abstraction, DAO & governance
- Full test suites: unit tests, fuzzing, integration tests, gas reports
- Preparing for remote junior roles/internships as smart contract developer

---

## 🛠️ Skills & Tools

- **Smart Contract Development**: Solidity, Foundry
- **Testing**: Unit, integration, fuzz testing, gas optimization & debugging
- **DeFi & Oracles**: ERC-20/721/1155 Common Token Standards, Chainlink (Data Feeds, VRF, Automation, Functions)
- **Protocol Concepts**: Upgradeable contracts, oracle-based logic, access control
- **EVM Basics**: Storage layout, calldata, events, gas costs
- **Standards & Tooling**: OpenZeppelin, Git/GitHub Actions, EVM testnets

---

## 💻 Tech Stack

![Solidity](https://img.shields.io/badge/Solidity-363636?style=for-the-badge&logo=solidity&logoColor=white) ![JavaScript](https://img.shields.io/badge/javascript-%23323330.svg?style=for-the-badge&logo=javascript&logoColor=%23F7DF1E) ![Ethereum](https://img.shields.io/badge/Ethereum-3C3C3D?style=for-the-badge&logo=ethereum&logoColor=white) ![Chainlink](https://img.shields.io/badge/Chainlink-375BD2?style=for-the-badge&logo=chainlink&logoColor=white) ![Foundry](https://img.shields.io/badge/Foundry-grey?style=for-the-badge&logo=data:image/png;base64,%3D&link=https%3A%2F%2Fbook.getfoundry.sh%2F)

---

## 🏅 Certifications & Training

**Cyfrin Updraft — Smart Contract Engineering Track**

- Blockchain Basics ✅
- Solidity Smart Contract Development ✅
- Foundry Fundamentals ✅
- Chainlink Fundamentals ✅
- Full-Stack Web3 Development ✅
- Advanced Foundry (In Progress) 🚀

---

## 🎯 Current Learning Focus

- Upgradeable contracts (Transparent & UUPS proxies)
- Account Abstraction (ERC-4337 basics)
- DAO/governance contracts (voting, proposals, execution)
- Security fundamentals (vulnerabilities, safer code)
- Advanced Foundry (scripting, fuzzing, invariants)

---

## 🔗 Connect With Me

<p align="center">
  <a href="https://linkedin.com/in/0xeunum">
    <img src="https://img.shields.io/badge/LinkedIn-%230077B5.svg?logo=linkedin&logoColor=white" />
  </a>
  &nbsp;
  <a href="https://x.com/0xEunum">
    <img src="https://img.shields.io/badge/X-black.svg?logo=X&logoColor=white" />
  </a>
  &nbsp;
  <a href="https://t.me/x0eunum">
    <img src="https://img.shields.io/badge/Telegram-2CA5E0?&logo=telegram&logoColor=white" />
  </a>
  &nbsp;
  <a href="mailto:0xeunum@gmail.com">
    <img src="https://img.shields.io/badge/Email-D14836?logo=gmail&logoColor=white" />
  </a>
</p>

---

## 📊 GitHub Activity

<p align="center">
  <img src="https://awesome-github-stats.azurewebsites.net/user-stats/0xEunum?cardType=github&theme=dark" />
  <br/>
  <img src="https://github-readme-stats-fast.vercel.app/api/top-langs/?username=0xeunum&layout=compact&theme=dark" />
</p>

---

> 🫱🏼‍🫲🏼 Let's build secure, production-grade Web3 protocols together.
